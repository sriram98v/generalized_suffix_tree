    ...
    U: Display + Debug + Eq + PartialEq + Hash + Clone,
{
    ...
    fn get_node(&self, q_string:&Vec<T>)->Option<&usize>{
        ...
        loop {
            match self.nodes.get(node_id).unwrap().get_child(c){
                ...
                Some(n) => {
                    ...
                    c = &q_string[i];
                    let mut j = 1;
                    while i < q_string.len() && j < self.nodes.get(n)?.edge_length(0){
                        if c != self.strings.get(self.nodes.get(n)?.get_string_id()).unwrap().get_string()[self.nodes.get(n)?.get_start() + j]{
                            return None;
                        }
                        if i==q_string.len()-1{
                            return Some(n.clone());
