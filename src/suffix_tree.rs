    ...
    U: Display + Debug + Eq + PartialEq + Hash,
{
    ...
    fn _walk_down(&mut self, next_node:Rc<RefCell<Node<T, U>>>, string:&Vec<T>, leaf_end:usize, mut active_length: usize, mut active_edge_index: usize, mut active_edge: Option<T>, mut active_node: Rc<RefCell<Node<T, U>>>)->bool{
        let edge_length = next_node.borrow().edge_length(leaf_end);
        if active_length >= edge_length{
            active_length -= edge_length;
            active_edge_index += edge_length;
            active_edge = Some(string[active_edge_index as usize]);
