...
fn add_string_set(){
    ...
    let sstring: HashMap<String, HashSet<usize>> = tree.find(&"CXYZ".chars().collect());
    ...
    let sstring: HashMap<String, HashSet<usize>> = tree.find(&"XYZT".chars().collect());
    assert_eq!(sstring, HashMap::from([("second".to_string(), HashSet::from([1]))]));
    let sstring: HashMap<String, HashSet<usize>> = tree.find(&"GATTA".chars().collect());
